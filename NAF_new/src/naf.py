...
from NAF_new.src.utils import get_timestamp
...
class ReplayBufferPER(PrioritizedReplayBuffer):
    ...
    def sample_batch(self, batch_size=32):
        if self.size <batch_size:
            ...
            obs1, acts, rews, obs2, done, gammas, weights, idxs = super(ReplayBufferPER, self).sample(batch_size, self.beta)
...
class NAF(object):
    def __init__(self, sess,
        ...
        self.x_ph, self.a_ph, self.mu, self.V, self.Q, self.P, self.A, self.vars_pred \
            = core.mlp_normalized_advantage_function(env.observation_space.shape, act_dim=env.action_space.shape, **nafnet_kwargs,
                                      scope='main')
        ...
        polyak = 1-tau
        self.target_update = tf.group([tf.assign(v_targ, (1 - polyak) * v_targ + (1 - polyak) * v_main)
                                  for v_main, v_targ in zip(self.vars_pred, self.vars_targ)])
        self.losses = []
        self.vs = []
    ...
    def run(self, is_train=True):
        ...
        for self.idx_episode in range(self.max_episodes):
            print('Epsisode number: ', self.idx_episode)
    ...
    def perceive(self):
        ...
        for iteration in range(self.update_repeat):
            ...
            target_y = priority_info[0]*(self.discount * np.squeeze(v) + r)
